...
const contactItems = [
  ...
  { key:"email",     value:EMAIL,                    href:`mailto:${EMAIL}`,  color:"#11999e", icon:<MailIcon/> },
...
export default function Contact() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const res = await fetch("/api/contact",{ method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({...data,type:"contact",captchaToken:"dev-bypass"}) });
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            ...
            {contactItems.map((item,i) => (
              <ScrollReveal key={item.key} delay={((i%3)+1) as 1|2|3}>
                <a href={item.href} target="_blank" rel="noopener noreferrer"
                  className="group flex items-center gap-4 p-5 bg-white rounded-2xl border transition-all duration-300"
                  ...
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--brand)" strokeWidth="2" className="flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <ScrollReveal delay={2}>
            <div className="bg-white rounded-2xl p-8 border" style={{ borderColor:"rgba(17,153,158,0.1)", boxShadow:"var(--shadow-lg)" }}>
              {/* Form header */}
              <div className="mb-6">
                <h3 className="font-heading text-xl font-bold mb-1" style={{ color:"var(--text-primary)" }}>{t("formTitle")}</h3>
              ...
              {submitted ? (
                <div className="text-center py-12">
                  ...
                  <h4 className="font-heading text-xl font-bold mb-2" style={{ color:"var(--text-primary)" }}>{t("success")}</h4>
                  <p className="font-body text-sm" style={{ color:"var(--text-soft)" }}>{t("successDesc")}</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                  {error && <div className="bg-red-50 border border-red-200 text-red-700 text-sm font-body px-4 py-3 rounded-xl">{error}</div>}
                  ...
                  {errors.service&&<p className="text-red-500 text-xs">Required</p>}
                  <textarea {...register("message")} placeholder={t("message")} rows={4} className="inp" style={{ resize:"none" }}/>
                  <button type="submit" disabled={loading} className="btn-brand btn-shimmer w-full py-3.5 text-base justify-center">
                  ...
                  <p className="text-center text-xs font-body" style={{ color:"var(--text-soft)" }}>{t("privacy")}</p>
                </form>
              )}
            </div>
          </ScrollReveal>
